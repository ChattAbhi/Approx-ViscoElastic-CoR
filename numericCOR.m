...
% argurments, using the name keywords 'AbsTol', 'RelTol', and 'MaxIter'.
...
% setting the keyword 'ShowPlots' to  true, as 
% numericCOR(alp,bet,gam,gtil,...,'ShowPlots',true). Save the plots as .fig
% and .pdf files by specifying the filenames using the keyword 'figsave' 
% and 'pdfsave', respectively as, 
...
% 'filename_phase.pdf', respectively. The state-trajectory data for the
% numerical integration can be obtained by setting the Name-Value pair
% keyword 'OutData' to true, as in the example:
% [e,T,U,dU]=numericCOR(...,'OutDat',true), where T yields a vector of the
% scaled times, U yields the corresponding scaled displacements and dU
% yields the corresponding scaled speeds.
...
Atol=1e-14; Rtol=1e-10; Etol=1e-8; miter=100; pdfname=[]; figname=[]; mult=false; ignore=false; outDat=false;
if ~isempty(varargin)
...
while iter<=max_iter
    tspan=[tin,tfin];
    options=odeset('AbsTol',Atol,'RelTol',Rtol,'Event',@(t,x)cor_event(t,x,gam,bet,alp,gtil,inter,Etol));
...
function [val,ister,dir]=cor_event(t,x,gam,beta,alp,gtil,inter,tol)
u=x(1,1);
uplus=max([u,0]);
udot=x(2) - gam*uplus^beta;
if inter==1
    val=[udot;max([abs(u-gtil^(1/alp)),abs(udot)])-tol;1];
    ister=[1;1;0];
    dir=[-1;0;0];
elseif inter==2 
    val=[u-gtil^(1/alp);max([abs(u-gtil^(1/alp)),abs(udot)])-tol;1];
    ister=[1;1;0];
    dir=[-1;0;0];
else
    val=[u*udot;u;udot];
