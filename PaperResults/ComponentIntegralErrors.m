function ComponentIntegralErrors
alp=3/2; 
...
for i=1:length(bet)
    ...
    for j=1:length(the)
        ...
        Mstore(1,j)=M;
        Mstore(2,j)=Mapprox(alp,bet(i),the(j),'Cheb');
...
xlabel('$\beta$','Interpreter','latex','FontSize',16)
savefig(strcat(prefixA,'CompMmaxRelErr.fig'));
print(strcat(prefixB,'CompMmaxRelErr.pdf'),'-dpdf');
...
ylabel('Max. Relative Error, $\mathcal{Q}(\tilde{g}(\theta))$','Interpreter','latex','FontSize',16)
xlabel('$\beta$','Interpreter','latex','FontSize',16)
savefig(strcat(prefixA,'CompQmaxRelErr.fig'));
print(strcat(prefixB,'CompQmaxRelErr.pdf'),'-dpdf');
