...
for i=1:length(gtil_choice)
    ...
    for j=1:length(gam_range)
        ...
        E(3,j)=sqrt(esqr); %O(gamma^2) COR, as \sqrt(e^2), with numerical I(gtilde) and Q(gtilde)
    ...
    for j=1:length(gam_range_z)
        ...
        Ez(3,j)=sqrt(esqr); %O(gamma^2) COR, as \sqrt(e^2), with numerical I(gtilde) and Q(gtilde)
